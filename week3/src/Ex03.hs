...
mysterious Leaf = []
mysterious (Branch j l r) = mysterious l ++ [j] ++ mysterious r
...
getMiddle xs = xs !! n where n = length xs `div` 2

splitList :: [Integer] -> (Integer, [Integer], [Integer])
splitList xs = ((xs !! n), (take n xs), (drop (n+1) xs)) 
              where n = length xs `div` 2

astonishing :: [Integer] -> BinaryTree
astonishing [] = Leaf
astonishing xs = Branch val (astonishing l) (astonishing r)
  where (val, l, r) = splitList xs
